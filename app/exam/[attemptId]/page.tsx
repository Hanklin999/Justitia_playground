...
export default function ExamPage() {
  ...
  const submitAttempt = useCallback(async () => {
    ...
    setSubmitting(true);
    setErrorMessage("");
    const { error } = await getSupabaseBrowserClient().rpc("submit_exam_attempt", { p_attempt_id: attemptId });
    if (error) {
      setSubmitting(false);
  ...
  const flushQuestionTime = useCallback(async () => {
    ...
    if (seconds < 1) return;
    await getSupabaseBrowserClient().rpc("add_question_active_seconds", {
  ...
  async function persistAnswer(nextAnswer: string) {
    if (!currentQuestion) return;
    const normalized = normalizeAnswer(nextAnswer);
    setAnswers((current) => {
      const copy = { ...current };
      if (normalized) copy[currentQuestion.question_id] = normalized;
  ...
  async function saveAnnotation(questionId: string, nextStar: boolean, nextNote: string) {
    ...
    const supabase = getSupabaseBrowserClient();
    const { error } = isReviewAttempt
      ? await supabase.rpc("save_review_annotation", {
          p_question_id: questionId,
          p_is_starred: nextStar,
          p_note_text: nextNote,
        })
      : await supabase.rpc("save_exam_annotation", {
          p_attempt_id: attemptId,
          p_question_id: questionId,
          p_is_starred: nextStar,
          p_note_text: nextNote,
  ...
  const optionMap: Record<AnswerChoice, string | null> = {
  ...
  const choices = allChoices.filter((choice) => optionMap[choice]);
  const selected = answers[currentQuestion.question_id] ?? "";
  const progress = answeredCount / payload.questions.length * 100;
  const annotationPrefix = isReviewAttempt ? "檢討" : "模考";

  return <div className="exam-shell">
    <header className="exam-header">
      <div>
        <div className="exam-kicker">
          {payload.attempt.attempt_mode === "official_paper" && payload.paper
            ? `${payload.paper.exam_year_roc} 年第 ${payload.paper.paper_order} 卷`
            : payload.attempt.attempt_mode === "wrong_review" ? "複習組卷" : "科目自組測驗"}
        </div>
    ...
    <main className="exam-layout">
      ...
      <section className="question-panel">
        ...
        <div className="question-tags">
          <span>{currentQuestion.subject_primary}</span>
          {currentQuestion.subsubject_primary !== "未分類" && <span>{currentQuestion.subsubject_primary}</span>}
          <span>{currentQuestion.question_type === "multiple_choice" ? `複選題・${currentQuestion.question_points} 分` : `單選題・${currentQuestion.question_points} 分`}</span>
        </div>
        <p className="question-text">{currentQuestion.question_text}</p>
        {currentQuestion.question_type === "multiple_choice" && <p className="multi-choice-hint">複選題：可選擇多個選項；105–106 年依官方規則計算部分得分。</p>}
        <div className="answer-options">
          {choices.map((choice) => <label className={`answer-option ${selected.includes(choice) ? "selected" : ""}`} key={choice}>
            <input
    ...
    <footer className="exam-footer">
      <span>{payload.attempt.is_timed ? `本次時長：${payload.attempt.duration_minutes} 分鐘` : "複習組卷不計時"}</span>
